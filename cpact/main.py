...
def discover_tests(test_dir, filters, logger=None):
    """
    ...
    for root, _, files in os.walk(test_dir):
        for file in files:
            if not (file.endswith(".yaml") or file.endswith(".yml") or file.endswith(".json")):
                continue
            ...
            try:
                ...
                if filters:
                    if filters.test_id and filters.test_id != metadata.get("test_id"):
                        continue
                    if filters.test_name and filters.test_name.lower() not in metadata.get("test_name", "").lower():
                        continue
                    if filters.test_group and filters.test_group != metadata.get("test_group"):
                        continue
                    if filters.tags and not set(filters.tags).intersection(set(metadata.get("tags", []))):
                        continue
...
def run_test(file_path, workspace, logger=None):
    import json
    ...
    ResultCollector().get_instance().dump_results(os.path.join(TestLogger().get_log_dir(), "test_results.json"))
    ResultCollector().get_instance().print_summary_table()
    logger.info(f"⏱️ Total execution time: {elapsed_time:.2f}s\n")
    

def list_tests(test_files, logger=None):
...
def list_scenarios_with_connections(test_files, connections, logger=None):
    ...
    def check_scenario_connections(scenario_connection_details, connections):
...
def discover_and_test_connections(config: Dict[str, Any], timeout: int = 10, export_csv: bool = False) -> Dict[str, Any]:
    """
...
def calculate_connection_statistics(test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    successful_results = [r for r in test_results if r['status'] == 'SUCCESS' and r['total_time']]
...
def validate_schema(schema_type: str, schema_file: str, file_or_dir: str, logger: TestLogger) -> bool:
    """
    ...
    if schema_type not in ["config", "scenario"]:
        ...
        return False
    schema_file = schema_file[0] if schema_file else os.path.join(os.path.dirname(os.path.dirname(__file__)), "spec", "schema_checker", f"{schema_type}_schema.json")
...
def main():
    ...
    parser.add_argument("--test_dir", type=str, required=True, help="Path to the root tests directory")
    parser.add_argument("--workspace", type=str, required=True, help="Directory for logs, JSON, and results")
    ...
    parser.add_argument("--conn-config", "-cc", type=str, help="Path to the connection config file")
    ...
    parser.add_argument("--run-with-discover-connections", "-rdc", default=False, action="store_true", help="Discover and test all connections before running tests")
    parser.add_argument(
        "--schema-check",
        nargs="+",
        help=(
    ...
    parser.add_argument("--run_with_schema_check", "-rsc", default=False, action="store_true", help="Run tests with schema check")
    args = parser.parse_args()
    
    log_path = os.path.join(args.workspace, "logs") or \
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "workspace", "logs")
    TestLogger(log_dir=log_path)
    logger = TestLogger().get_logger()
    log_dir = TestLogger().get_log_dir()
    logger.info(f"Log directory: {log_dir}")
    logger.info(f"Starting test discovery in: {args.test_dir}")
    
    all_tests_list = discover_tests(args.test_dir, None, logger=logger)
    ...
    matched_files = discover_tests(args.test_dir, args, logger=logger)
    for file_path in matched_files:
        run_test(file_path, args.workspace, logger=logger)
