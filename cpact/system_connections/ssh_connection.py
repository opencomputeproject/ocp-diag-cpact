...
from system_connections.constants import ExecutionMode, TaskStatus, TaskResult, BackgroundTask
...
class SSHConnection(ConnectionInterface):
    ...
    def download_file(self, remote_path: str, local_path: str) -> bool:
        ...
        if not self.is_connected():
            if not self.connect():
                self.logger.error("Failed to establish SSH connection for file download")
                return False
        ...
        try:
            ...
            sftp.close()
            self.logger.info(f"File downloaded successfully: {remote_path} -> {local_path}")
            return True
        except Exception as e:
            tb_str = ''.join(traceback.format_exception(type(e), e, e.__traceback__))
            self.logger.error(f"Formatted traceback:\n{tb_str}")
    ...
    def upload_file(self, local_path: str, remote_path: str) -> bool:
        ...
        try:
            ...
            sftp.close()
            self.logger.info(f"File uploaded successfully: {local_path} -> {remote_path}")
            return True
        except Exception as e:
            tb_str = ''.join(traceback.format_exception(type(e), e, e.__traceback__))
            self.logger.error(f"Formatted traceback:\n{tb_str}")
    ...
    def execute_command(self, command: str, mode: ExecutionMode = ExecutionMode.SYNCHRONOUS, 
                       timeout: Optional[float] = None, wait: bool = False) -> Optional[TaskResult]:
        """
    ...
    def _execute_synchronous(self, task_id: str, command: str, timeout: Optional[float] = None) -> TaskResult:
        """Execute command synchronously and return result immediately."""
        start_time = time.time()
        result = TaskResult(task_id=task_id, command=command, status=TaskStatus.RUNNING, start_time=start_time)
        ...
        try:
            ...
            try:
                ...
                result.status = TaskStatus.COMPLETED if exit_code == 0 else TaskStatus.FAILED
    ...
    def _execute_background(self, task_id: str, command: str):
    ...
    def _background_worker(self, task_id: str, command: str):
        ...
        try:
            ...
            while True:
                ...
                with self.lock:
                    if task_id in self.background_tasks and self.background_tasks[task_id].terminated:
            ...
            try:
                with self.lock:
                    if task_id in self.background_tasks and not self.background_tasks[task_id].terminated:
                        exit_code = stdout.channel.recv_exit_status()
            except Exception as e:
                tb_str = ''.join(traceback.format_exception(type(e), e, e.__traceback__))
                self.logger.error(f"Formatted traceback:\n{tb_str}")
            ...
            with self.lock:
                is_terminated = task_id in self.background_tasks and self.background_tasks[task_id].terminated
        ...
        self.logger.info(f"Background task {task_id} completed with status: {result.status}")
        return result
    ...
    def wait_for_task(self, task_id: str, timeout: Optional[float] = None) -> Optional[TaskResult]:
        ...
        try:
            ...
            self.logger.warning(f"Timeout waiting for task {actual_task_id}, terminating and collecting results")
            ...
            actual_task_id = str(task_id) if not hasattr(task_id, 'task_id') else task_id.task_id
            return TaskResult(
                task_id=actual_task_id,
    ...
    def wait_for_all_tasks(self, timeout: Optional[float] = None) -> Dict[str, TaskResult]:
        """
    ...
    def wait_for_task_with_details(self, task_id: str, timeout: Optional[float] = None, 
                                  wait: bool = True) -> Optional[Tuple[str, str, int, str]]:
        """
        ...
        if not wait:
            ...
            if details:
                return (details.get('stdout', ''), details.get('stderr', ''), 
                       details.get('return_code', -1), details.get('status', 'unknown'))
    ...
    def terminate_task(self, task_id: str) -> bool:
        ...
        with self.lock:
            if actual_task_id not in self.background_tasks:
                self.logger.warning(f"Task {actual_task_id} not found or already completed")
                return False
        ...
        try:
            ...
            try:
                ...
                tb_str = ''.join(traceback.format_exception(type(cleanup_error), cleanup_error, cleanup_error.__traceback__))
                self.logger.error(f"Formatted traceback:\n{tb_str}")
                self.logger.error(f"Failed to create terminated result: {cleanup_error}")
    ...
    def get_task_result(self, task_id: str, wait: bool = False, timeout: float = None, 
                       force_stop: bool = False) -> Optional[Tuple[str, str, int, str]]:
        """
    ...
    def get_all_background_results(self, wait: bool = True, timeout: float = None, 
                                  force_stop_remaining: bool = True) -> Dict[str, Tuple[str, str, int, str]]:
        """
        ...
        if wait:
            ...
            for task_id, result in all_results.items():
                results[task_id] = (result.stdout, result.stderr, result.return_code, result.status.value)
        else:
        ...
        if force_stop_remaining:
            running_tasks = [tid for tid, (_, _, _, status) in results.items() if status == 'running']
