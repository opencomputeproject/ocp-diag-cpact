...
class ConnectionDiscovery:
        ...
        for connection_name in available_connections:
            if connection_name in self.config:
                ...
                for conn_type in available_types:
                    ...
                    if self._is_valid_combination(connection_name, conn_type, conn_config):
                        discovered_connections.append({
                            'connection_name': connection_name,
                            'connection_type': conn_type,
                            'config_available': True,
                            'description': self._get_connection_description(connection_name, conn_type, conn_config)
                        })
    ...
    def _is_valid_combination(self, connection_name: str, conn_type: str, conn_config: Dict) -> bool:
    ...
    def _get_connection_description(self, connection_name: str, conn_type: str, conn_config: Dict) -> str:
        ...
        if conn_type == 'ssh':
            ...
            tunnel_info = ""
            if connection_name == 'NodeManager' and conn_config.get('nodemanager_tunnel'):
    ...
    def test_all_connections(self, timeout: int = 10) -> List[Dict[str, Any]]:
        ...
        print(f"Testing {discovery_result['total_combinations']} connection combinations...")
        ...
        for i, conn_info in enumerate(discovery_result['connections'], 1):
            ...
            print(f"Testing {i}/{discovery_result['total_combinations']}: {connection_name} ({connection_type})")
    ...
    def _test_single_connection(self, connection_name: str, connection_type: str, description: str, timeout: int) -> Dict[str, Any]:
        """Test a single connection"""
        start_time = time.time()
        
        try:
            # Create connection
            connection = self.factory.create_connection(connection_name, connection_type)
            ...
            command_result = self._execute_test_command(connection, connection_type, timeout)
    ...
    def _execute_test_command(self, connection: ConnectionInterface, connection_type: str, timeout: int) -> Dict[str, Any]:
        """Execute appropriate test command based on connection type"""
        try:
            if connection_type == 'ssh':
                result = connection.execute_command('echo "SSH test successful"', timeout=timeout)
                return {
                ...
                result = connection.execute_command('echo "Local test successful"', shell=True, timeout=timeout)
                return {
                    'success': result.get('success', False),
                    'output': result.get('stdout', ''),
                    'error': result.get('stderr', '') or result.get('error', '')
    ...
    def print_connection_table(self, test_results: List[Dict[str, Any]]):
        ...
        sorted_results = sorted(test_results, key=lambda x: status_order.get(x['status'], 5))
        ...
        for result in sorted_results:
            ...
            # Format timing
            connect_time = f"{result['connect_time']:.2f}s" if result['connect_time'] else "N/A"
            command_time = f"{result['command_time']:.2f}s" if result['command_time'] else "N/A"
        ...
        print(f"SUMMARY: Total: {total_tests} | ✅ Success: {successful} | ⚠️  Partial: {partial} | ❌ Failed: {failed} | 💥 Error: {errors}")
    ...
    def export_results_to_csv(self, test_results: List[Dict[str, Any]], filename: str = "connection_test_results.csv"):
        """Export test results to CSV file"""
        try:
            import csv
            
            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['connection_name', 'connection_type', 'status', 'description', 
                            'connect_time', 'command_time', 'total_time', 'error', 'details']
